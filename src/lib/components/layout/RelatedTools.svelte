...
<section
	class="mt-12 border-t border-slate-100 pt-10 sm:mt-20 sm:pt-16 dark:border-slate-800 print:hidden"
>
	<div
		class="mb-8 flex flex-col items-start justify-between gap-4 sm:mb-10 sm:flex-row sm:items-center"
	>
		<div>
			...
			<p class="mt-1 text-sm text-slate-500 sm:text-base dark:text-slate-400">
				Jelajahi ekosistem alat profesional Kerjao.
			</p>
		</div>
		<a
			href={safeResolve('/semua-alat')}
	...
	<div class="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
		{#each allSuggestions as tool (tool.path)}
			...
			>
				<div
					class="mb-4 flex h-14 w-14 items-center justify-center rounded-2xl bg-slate-50 text-3xl shadow-inner transition-all group-hover:bg-blue-50 dark:bg-slate-800 dark:group-hover:bg-blue-900/30"
				>
					{tool.icon}
				</div>
				<h3
					class="mb-2 text-lg font-bold text-slate-900 group-hover:text-blue-600 dark:text-white dark:group-hover:text-blue-400"
				>
					{tool.name}
				</h3>
				...
				<div class="mt-auto flex w-full items-center justify-between">
					...
					>
					<div
						class="flex h-8 w-8 items-center justify-center rounded-full bg-slate-50 text-slate-400 transition-all group-hover:bg-blue-600 group-hover:text-white dark:bg-slate-800"
					>
