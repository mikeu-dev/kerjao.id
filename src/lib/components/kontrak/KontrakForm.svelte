...
<div class="flex flex-col gap-8">
	...
	<div class="space-y-6">
		...
		{:else if activeSection === 'payroll'}
			<div in:slide={{ axis: 'y' }} class="space-y-4">
				<h3 class="text-xs font-bold uppercase tracking-wider text-slate-400">Upah, Bahasa & TTD</h3>
				
				<div class="space-y-2">
					<label for="k-lang" class="text-xs font-semibold text-slate-600 dark:text-slate-300">Bahasa Dokumen</label>
				...
				<div class="grid grid-cols-2 gap-4">
					...
					<div class="space-y-2">
						<label for="k-ttd-date" class="text-xs font-semibold text-slate-600 dark:text-slate-300">Tanggal Tanda Tangan</label>
						<input id="k-ttd-date" type="date" bind:value={data.tanggalTtd} class="w-full rounded-xl border-slate-200 bg-white px-4 py-2.5 text-sm focus:border-indigo-500 focus:ring-indigo-500 dark:border-slate-700 dark:bg-slate-900 dark:text-white" />
					</div>
				</div>
