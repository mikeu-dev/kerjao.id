<script lang="ts">
	...
	async function copyToClipboard() {
		try {
			isCopying = true;
			const textToCopy = `Halo, Kwitansi (Tanda Terima) telah berhasil dibuat melalui kerjao.id. Anda dapat mengunduhnya dalam format PDF di halaman aplikasi secara instan.`;
	...
	function printSurat() {
		// Adds a slight timeout to ensure styles apply before printing
		setTimeout(() => {
			window.print();
		}, 100);
	}
...
<div class="flex flex-col gap-3 sm:flex-row">
	<button
		onclick={printSurat}
		class="group relative flex w-full items-center justify-center gap-2 overflow-hidden rounded-2xl bg-emerald-600 px-6 py-3.5 font-semibold text-white shadow-[0_0_20px_-5px_rgba(16,185,129,0.3)] transition-all duration-300 hover:-translate-y-0.5 hover:bg-emerald-700 hover:shadow-[0_0_25px_-5px_rgba(16,185,129,0.4)] focus:ring-2 focus:ring-emerald-500 focus:ring-offset-2 focus:outline-none sm:w-auto dark:focus:ring-offset-slate-900"
	>
		<span class="relative z-10">Unduh Kwitansi (PDF)</span>
		<svg
	...
	>
		<span>{isCopying ? 'Tersalin!' : 'Bagikan Info Info'}</span>
		{#if !isCopying}
			<svg
