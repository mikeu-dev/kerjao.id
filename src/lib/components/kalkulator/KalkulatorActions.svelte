<script lang="ts">
	...
	async function copyToClipboard() {
		try {
			isCopying = true;
			const textToCopy = `Estimasi Slip Gaji\n\nPenghasilan Bruto: ${formatRupiah(result.penghasilanBruto)}\nTotal Potongan: - ${formatRupiah(result.totalPotongan)}\n---------------------------\nTAKE HOME PAY: ${formatRupiah(result.takeHomePay)}\n\nDihitung via kerjao.id`;
...
<div class="flex flex-col gap-3 sm:flex-row">
	...
	>
		<span class="relative z-10">{isCopying ? 'Berhasil Disalin!' : 'Salin Rincian'}</span>
		{#if !isCopying}
			<svg
