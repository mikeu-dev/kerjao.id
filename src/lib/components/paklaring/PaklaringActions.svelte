<script lang="ts">
	...
	async function copyToClipboard() {
		try {
			isCopying = true;
			const textToCopy = `Halo, saya telah membuat draf Surat Keterangan Kerja (Paklaring) di kerjao.id. Anda bisa membuat dan mencetaknya langsung melalui website.`;
...
<div class="flex flex-col gap-3 sm:flex-row">
	...
	>
		<span class="relative z-10">Cetak / Simpan PDF</span>
		<svg
	...
	>
		<span>{isCopying ? 'Disalin!' : 'Bagikan Info'}</span>
		{#if !isCopying}
			<svg
