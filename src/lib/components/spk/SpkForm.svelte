...
<div class="flex flex-col gap-8">
	...
	<div class="space-y-6">
			...
			<div in:slide={{ axis: 'y' }} class="space-y-4">
				...
				<div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
					<div class="space-y-2">
						...
						<input
							id="p1-perusahaan"
							type="text"
							bind:value={data.pemberiKerja.perusahaan}
							placeholder="PT Maju Bersama"
							class="w-full rounded-xl border-slate-200 bg-white px-4 py-2.5 text-sm focus:border-blue-500 focus:ring-blue-500 dark:border-slate-700 dark:bg-slate-900 dark:text-white"
						/>
					</div>
					<div class="space-y-2">
						<label for="p1-nama" class="text-xs font-semibold text-slate-600 dark:text-slate-300"
							>Penanggung Jawab / Jabatan</label
						>
						<input
							id="p1-nama"
							type="text"
							bind:value={data.pemberiKerja.nama}
							placeholder="Budi Santoso (Direktur)"
							class="w-full rounded-xl border-slate-200 bg-white px-4 py-2.5 text-sm focus:border-blue-500 focus:ring-blue-500 dark:border-slate-700 dark:bg-slate-900 dark:text-white"
						/>
					</div>
				</div>
				<div class="space-y-2">
					...
					<input
						id="p1-alamat"
						type="text"
						bind:value={data.pemberiKerja.alamat}
						placeholder="Jl. Raya Bisnis No. 12"
						class="w-full rounded-xl border-slate-200 bg-white px-4 py-2.5 text-sm focus:border-blue-500 focus:ring-blue-500 dark:border-slate-700 dark:bg-slate-900 dark:text-white"
					/>
				</div>
			</div>
		{:else if activeSection === 'pelaksana'}
			<div in:slide={{ axis: 'y' }} class="space-y-4">
				<h3 class="text-sm font-bold tracking-wider text-slate-400 uppercase">
					Data Pelaksana (Vendor/Freelancer)
				</h3>
				<div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
					<div class="space-y-2">
						<label
							for="p2-perusahaan"
							class="text-xs font-semibold text-slate-600 dark:text-slate-300"
							>Nama Pelaksana / CV</label
						>
						<input
							id="p2-perusahaan"
							type="text"
							bind:value={data.pelaksana.perusahaan}
							placeholder="Vendor Solusi Kreatif"
							class="w-full rounded-xl border-slate-200 bg-white px-4 py-2.5 text-sm focus:border-blue-500 focus:ring-blue-500 dark:border-slate-700 dark:bg-slate-900 dark:text-white"
						/>
					</div>
					<div class="space-y-2">
						...
						<input
							id="p2-nama"
							type="text"
							bind:value={data.pelaksana.nama}
				...
				<div class="space-y-2">
					...
					<input
						id="p2-alamat"
						type="text"
						bind:value={data.pelaksana.alamat}
		...
		{:else if activeSection === 'proyek'}
			<div in:slide={{ axis: 'y' }} class="space-y-4">
				<h3 class="text-sm font-bold tracking-wider text-slate-400 uppercase">Detail Proyek</h3>
				<div class="space-y-2">
					<label for="pro-nama" class="text-xs font-semibold text-slate-600 dark:text-slate-300"
						>Nama Proyek / Pekerjaan</label
					>
					<input
						id="pro-nama"
						type="text"
						bind:value={data.proyek.nama}
						placeholder="Contoh: Pengembangan Website Company Profile"
						class="w-full rounded-xl border-slate-200 bg-white px-4 py-2.5 text-sm focus:border-blue-500 focus:ring-blue-500 dark:border-slate-700 dark:bg-slate-900 dark:text-white"
					/>
				</div>
				<div class="space-y-2">
					<label for="pro-lingkup" class="text-xs font-semibold text-slate-600 dark:text-slate-300"
						>Ruang Lingkup (Scope) Pekerjaan</label
					>
					<div class="flex gap-2">
						...
						<button
							onclick={addScope}
							class="rounded-xl bg-blue-600 px-4 text-white hover:bg-blue-700">Tambah</button
						>
					</div>
						...
						{#each data.proyek.ruangLingkup as scope, i}
							...
							>
								...
								<button
									...
									aria-label="Hapus Poin Pekerjaan"
								>
		...
		{:else if activeSection === 'biaya'}
			<div in:slide={{ axis: 'y' }} class="space-y-4">
				<h3 class="text-sm font-bold tracking-wider text-slate-400 uppercase">Jadwal & Biaya</h3>
				<div class="grid grid-cols-2 gap-4">
					<div class="space-y-2">
						<label for="w-mulai" class="text-xs font-semibold text-slate-600 dark:text-slate-300"
							>Tanggal Mulai</label
						>
						<input
							id="w-mulai"
							type="date"
							bind:value={data.waktu.mulai}
							class="w-full rounded-xl border-slate-200 bg-white px-4 py-2.5 text-sm focus:border-blue-500 focus:ring-blue-500 dark:border-slate-700 dark:bg-slate-900 dark:text-white"
						/>
					</div>
					<div class="space-y-2">
						<label for="w-selesai" class="text-xs font-semibold text-slate-600 dark:text-slate-300"
							>Target Selesai</label
						>
						<input
							id="w-selesai"
							type="date"
							bind:value={data.waktu.selesai}
							class="w-full rounded-xl border-slate-200 bg-white px-4 py-2.5 text-sm focus:border-blue-500 focus:ring-blue-500 dark:border-slate-700 dark:bg-slate-900 dark:text-white"
						/>
					</div>
				</div>
				<div class="space-y-2">
					<label for="b-nominal" class="text-xs font-semibold text-slate-600 dark:text-slate-300"
						>Nilai Kontrak (Nominal IDR)</label
					>
					<div class="relative">
						...
						<input
							id="b-nominal"
							type="number"
							bind:value={data.biaya.nominal}
							placeholder="0"
							class="w-full rounded-xl border-slate-200 bg-white py-2.5 pr-4 pl-10 text-sm focus:border-blue-500 focus:ring-blue-500 dark:border-slate-700 dark:bg-slate-900 dark:text-white"
						/>
					</div>
				</div>
				<div class="space-y-2">
					<label for="b-catat" class="text-xs font-semibold text-slate-600 dark:text-slate-300"
						>Catatan Pembayaran (Opsional)</label
					>
					<input
						id="b-catat"
						type="text"
						bind:value={data.biaya.catatan}
						placeholder="Misal: Pembayaran bertahap 30-70%"
						class="w-full rounded-xl border-slate-200 bg-white px-4 py-2.5 text-sm focus:border-blue-500 focus:ring-blue-500 dark:border-slate-700 dark:bg-slate-900 dark:text-white"
					/>
				</div>
			</div>
