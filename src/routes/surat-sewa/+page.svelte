...
<div class="relative z-10 mx-auto max-w-7xl px-4 pt-20 pb-12 sm:px-6 lg:px-8 lg:pt-32">
	...
	<div class="mb-12 text-center print:hidden">
		<h1 class="mb-4 text-4xl font-extrabold tracking-tight sm:text-5xl lg:text-6xl">
			<span class="bg-linear-to-r from-indigo-500 to-blue-600 bg-clip-text text-transparent"
				>Surat Perjanjian Sewa</span
			> Properti
		</h1>
		<p class="mx-auto mt-4 max-w-2xl text-lg text-slate-600 sm:text-xl dark:text-slate-400">
			Lindungi properti Anda dengan draf hukum yang jelas. Cukup isi data, lalu cetak ke format PDF
			dalam sekejap.
		</p>
	</div>
	...
	<div class="grid grid-cols-1 gap-8 lg:grid-cols-12 lg:gap-12">
		...
		<div class="flex flex-col gap-6 lg:col-span-12 xl:col-span-5 print:hidden">
			<div
				class="rounded-3xl border border-slate-200/60 bg-white/60 p-6 shadow-xl backdrop-blur-xl sm:p-8 dark:border-slate-700/60 dark:bg-slate-800/60"
			>
				<SewaForm bind:data />
			</div>
			<div
				class="rounded-3xl border border-slate-200/60 bg-white/40 p-6 shadow-sm backdrop-blur-xl dark:border-slate-700/60 dark:bg-slate-800/40"
			>
				<SewaActions />
			</div>
		</div>
		...
		<div class="flex flex-col gap-6 lg:col-span-12 xl:col-span-7">
			...
			>
				...
				<!-- Printable Area -->
				<div
					class="preview-container mx-auto overflow-hidden bg-white shadow-2xl print:shadow-none"
				>
					<SewaPreview {data} />
				</div>
			</div>
